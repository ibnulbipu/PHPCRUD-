	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<?php
				$del = mysqli_query($conn, "select * from Employee where userid='" . $row['userid'] . "'");
				$drow = mysqli_fetch_array($del);
				?>
				<div class="container-fluid">
					<h5>
						<center>Are you sure to delete <strong><?php echo ucwords($drow['em_name'] . ' ' . $row['em_email'] . '' . $row['contact_no'] . '' . $row['em_designation']); ?></strong> from the list? This method cannot be undone.</center>
					</h5>
				</div>
			</div>
...
<div class="modal fade" id="edit<?php echo $row['userid']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<?php
				$edit = mysqli_query($conn, "select * from Employee where userid='" . $row['userid'] . "'");
				...
				<div class="container-fluid">
					<form method="POST" action="edit.php?id=<?php echo $erow['userid']; ?>">
						<div class="row">
							...
							<div class="col-lg-10">
								<input type="text" name="em_name" class="form-control" value="<?php echo $erow['em_name']; ?>">
							</div>
						</div>
						<div style="height:10px;"></div>
						<div class="row">
							<div class="col-lg-2">
								<label style="position:relative; top:7px;">E-mail:</label>
							</div>
							<div class="col-lg-10">
								<input type="email" name="em_email" class="form-control" value="<?php echo $erow['em_email']; ?>">
							</div>
						</div>
						<div style="height:10px;"></div>
						<div class="row">
						...
						<div class="row">
							...
							<div class="col-lg-10">
								<input type="text" name="em_designation" class="form-control" value="<?php echo $erow['em_designation']; ?>">
							</div>
						</div>
				</div>
			</div>
